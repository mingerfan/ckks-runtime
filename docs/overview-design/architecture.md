...
> 本文描述项目架构。RuntimePlan V1 的 Encode、bundle、Host compute、OperatorSpec 校验和 Vec/Mock/MPI 验证路径已经落地；真实 Poseidon 后端属于后续阶段。
...
- 在消费某个值之前建立完成依赖；GPU 值优先在 stream 上等待；
- 运行结束前等所有未完成的发送收尾；
...
这里的 Ready/Pending 只描述 Runtime 是否已经拿到一个可交给 Api 的值句柄。
Ready 不保证 GPU 工作已经物理完成；句柄可以携带 CUDA event，消费方在自己的
stream 上等待。Pending 专指通信尚未产出本地句柄，主要用于不支持立即发布的旧
Api。
...
14. 使用一个值之前，必须先拿到 Ready 句柄；句柄携带的异步依赖由 Api 保序。
...
> 计算或通信函数返回本地值句柄后，后续 Api 调用必须能用它建立依赖。GPU Api
> 应把依赖放进 consumer stream，而不是让 CPU 等数据完成。Host 物化、旧 Api
> 的 Pending 值和运行结束仍允许显式等待。
...
- **Host 等待仍会阻塞调用线程**。GPU 后端通过立即发布带 event 的输出避免在普通消费者前执行这种等待；编译器仍应尽早发起通信，让 GPU 有更多重叠空间。
...
本节描述阶段一代码范围。更精确的状态见[实现状态](implementation-status.md)。
...
- 全部 V1 通信 hint 的合法性检查；
- inline/bundle Encode 和分 rank bundle 装载；
- OperatorSpec、能力、具体密钥和完整值元信息校验；
- Host compute、native Boot 和 `decrypt_reencrypt` Boot；
- 原始文件摘要与调试跳过策略；
- 一次性预加载；
- Ready/Pending 状态管理；
- 与单设备顺序流做最终结果对比；
