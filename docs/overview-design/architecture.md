...
Api 负责：定义值类型、实现 `encode_plaintext(ValueDesc, slots)`、实现各计算函数、实现 `communicate_async`、`wait` 和最终输出 `synchronize`、区别对待 Host 和 Device、选择或降级到等价的传输方式、转换 CPU/GPU 对象表示、抛出带细节的错误、实现 `abort_all`。
