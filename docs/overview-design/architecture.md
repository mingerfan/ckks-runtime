...
Api 负责：定义值类型、实现各计算函数、实现 `communicate_async`、`wait` 和最终输出 `synchronize`、区别对待 Host 和 Device、选择或降级到等价的传输方式、转换 CPU/GPU 对象表示、抛出带细节的错误、实现 `abort_all`。
...
4. 不存在隐式的跨 Place 操作数。
5. 所有数据搬运都由显式的通信动作表示。
6. 每个值都有完整的 context、level、`scale_log2`、NTT 和分量数,传输不改变这些元信息。
7. 算子的目标元信息与输出 ValueDesc 一致,并符合所引用的 OperatorSpec/profile。
8. Boot 的 `native` 或 `decrypt_reencrypt` 模式在计划中明确,不做运行时隐式回退。
9. 通信动作的数学语义和结果类型都明确。
10. 来源/目标 Place 都来自编译目标。
11. 传输 ID 在同一个计划里稳定且唯一。
12. 所有 rank 使用同一份可执行计划。
13. 使用一个值之前，必须先把它从 Pending 等成 Ready。
14. Api 的通信降级不得改变通信动作的数学语义或 Place。
15. 任意错误都会终止整个执行组。
