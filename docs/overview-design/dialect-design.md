...
当前 `dist` dialect 只实现 1 输入、1 输出的 `dist.transfer`。`materialize-ckks-communication` 为每条跨 Place SSA 边插入该 op，同一逻辑值到同一目标 Place 只生成一个副本，hint 固定为 `point_to_point`。Replicate 仍是 RuntimePlan 已支持但编译器尚未生成的后续优化。
...
把可执行 CKKS 序列化成 RuntimePlan V1 JSON。当前 `emit-runtime-plan` 支持单 block 的 Host 或已 placement 函数；`ckks.encode` 进入 initialization，`dist.transfer` 按来源进入 initialization 或 execution，其余计算进入 execution。已 placement 的原 CKKS 逻辑 ValueId 保持不变，Transfer 输出使用新 ValueId，target 写入真实 `world_size` 和 `device_counts`。导出器不重算 level/scale、不插入算子，遇到未消除的 Upscale、旧 `.cst` 索引、控制流或未知算子直接报错。JSON 是 Dacapo 与 Runtime 之间唯一的稳定协议。
...
Runtime 读完 V1 JSON 后,内部 C++ 类型至少要能表达下面这些信息。这里描述的是目标形态,不是跨仓库 ABI。当前 `runtime/plan.hpp`、reader、verifier 和 runtime 已支持单位置 SSA、三阶段计划、CKKS 元信息、OperatorSpec 引用、整数 `scale_log2`、`EncodeOp`、inline/bundle payload 以及 Encode capability。
