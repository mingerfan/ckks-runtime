...
- C++ 内存版的固定 target、三阶段 RuntimePlan、稳定指令序号/TransferId、内部 64 位计划摘要和计划打印。这个摘要供当前 Mock/MPI 多 rank 一致性检查使用,不是目标 V1 的原始文件 `plan_source_sha256`。
...
- MPI 环境自检和主机内存通信基准。
- RuntimePlan V1 上一版 JSON 读取器：严格拒绝 BOM、重复 key、未知/缺失字段、浮点数、非法枚举和错误的 64 位 ID 编码，并用仓库内 SHA-256 重算旧自嵌指纹；nlohmann-json 固定在 `third_party/`，不依赖 OpenSSL。它尚未支持当前规范中的显式 Encode、inline 浮点 payload 或原始文件字节摘要。
...
- 目标 Api 需要能按 ValueDesc 校验不透明值，并用读取阶段计算的 `plan_source_sha256`、摘要调试策略和 target/context/密钥要求做 preflight；当前接口只用内部 64 位摘要检查多 rank 计划一致性；
- 当前异步 `VecValue` 的 Pending 句柄只提前保存明文/密文类别；为支持非阻塞的完整 `validate_value`，后续还要让句柄在创建时携带不可变 CKKS 元信息；
- OperatorSpec 文件读取、id/版本/原始文件 `source_sha256` 匹配、level/rescale/boot profile 检查;
...
- RuntimePlan、OperatorSpec 和 manifest 的完整原始文件字节 SHA-256,以及 `skip_artifact_digest_checks` 调试策略;
