...
- 顶层可选 `plaintext_bundle` 引用的解析与外部输入 Host-only 检查(IO-2);但现有代码仍把 bundle 明文当 external input。显式 EncodeOp、双 payload、manifest `content` 查找、数据包装载和 BND-1 preflight 均未实现。
