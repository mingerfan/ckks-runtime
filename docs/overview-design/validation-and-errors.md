...
- **协议与后端**：计划 ID/版本、target、capability、OperatorSpec id/version/fingerprint、rescale/boot 模式一致；
- **部署输入**：本 rank 的 external_inputs 完整绑定，Api 已在启动前配置所需 context 和密钥；
- **多 rank 数据**：各 rank 使用同一计划指纹；存在 bundle Encode 时，都拿到 id/version/fingerprint 和完整 blob 集合一致的数据包。
