...
- bundle `content` 格式正确且能在 manifest 中找到;manifest 中所有 blob 的文件都存在,长度和 SHA-256 相符,能解码成不含 NaN、Infinity 或负零的非空小端 float64 数组;每个 Encode 再按自己的输出检查 slot 容量;
...
启动时检查：计划 ID/版本、节点总数、本地 rank、本地设备数、编译目标标识、capability 版本、OperatorSpec id/版本/指纹、rescale/boot 模式、本 rank 的调用方 external_inputs 已完整绑定、计划含 bundle Encode 时部署方已传入完整 bundle 目录且所有 content 校验通过、所有 rank 使用相同计划和同一 bundle fingerprint。
