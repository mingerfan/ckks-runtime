...
|  |- poseidon_cpu_api.*
|  |- poseidon_gpu_api.*
|  `- communication/            # 对象拆分、Host/Device、跨卡和跨进程通信
...
本节只解释跨仓库为什么需要这些信息，不再充当字段规范。字段名称、合法取值和拒绝条件以 [RuntimePlan V1](../runtime-plan/v1/specification.md) 与 [OperatorSpec V1](../operator-spec/v1/specification.md)为准。
...
Rescale 模式只由 OperatorSpec 声明，Boot 实现只由每条 Boot 指令及其 profile 声明。能力和密钥由 Runtime 从实际指令推导，不在计划头部重复保存。
...
本仓库的 RuntimePlan V1 已包含显式 Encode、inline/bundle payload、bundle 装载和完整 OperatorSpec 校验。协议中的 scale 仍只用整数 `scale_log2`；浮点数只出现在 Encode inline slot 数据。计划摘要直接覆盖文件原始字节，不做 JCS。
...
普通算子怎样改变 level、`scale_log2` 和 components，由 RuntimePlan 规范统一定义，不在 OperatorSpec 里重复。V1 不定义统一噪声单位，因此 `noise_by_level` 必须显式为 `null`。Dacapo 旧 SEAL profile 已有逐 level 噪声，HEAAN Boot 也有逐 level 延迟；这些数据不能无损塞进冻结的 V1，所以阶段二使用 [OperatorSpec V2](../operator-spec/v2/specification.md) 保存，并用 `dacapo-*` target 与 Poseidon profile 隔离。

CPU 和 GPU 使用不同的 OperatorSpec。Poseidon CPU 不要求 lazy-rescale,CPU profile 使用 eager 模式;当前 Poseidon GPU 的数据模数为 30 bit，GPU profile 明确使用 lazy 模式。也就是说,lazy-rescale Pass 对整个 Dacapo 编译器是可选项,但选中这个 GPU profile 后就是必选项。GPU boot 的 level 消耗不能沿用 CPU 数值,必须以所选 GPU boot profile 为准。如果内部实现或 RNS bit 限制改变了消耗,就更新 OperatorSpec 版本或 profile,不能悄悄改变同一版本的含义。

这里必须区分“模数链 level”和“bit 消耗”。Poseidon `GpuEvaluator::rescale` 每调用一次只丢掉一个 30-bit 的 RNS 模数，所以 RuntimePlan 的 `level` 下降 1，`scale_log2` 约下降 30。当前 lazy-rescale 的一次逻辑 Rescale 需要连续调用四次物理 rescale，因此是 `level` 下降 4、总模数与 scale 预算下降约 120 bit。文档统一写“4 个 level / 120 bit”，不写“120 个 level”。
...
- 当前 Poseidon GPU OperatorSpec 声明 `rescale_mode=lazy`,选中该 profile 时 Dacapo 必须启用对应 pass。当前 lazy Rescale 把四个 30-bit 的物理 rescale 合成一条逻辑 Rescale;
...
1. 从 OperatorSpec V2 按算子和输入 level 读取计算耗时；
2. 从 DAG 尾部向前计算优先级，关键路径更长的 ready 节点先分配；
3. 对每个候选 device，计算所有输入到达的最早时间，再在该 device 已占用的时间区间中寻找最早空位；
...
新增的 `dist` dialect 目前只有一条 `dist.transfer`：一个输入、一个输出、明确的 source/destination rank 和 device，以及稳定的 transfer id。`materialize-ckks-communication` 检查每条 compute 的操作数；只要生产位置和计算位置不同，就插入 Transfer，并把操作数改成新物理 ValueId。同一个逻辑值传到同一个目标 Place 时只生成一次，后面的消费者复用该副本。

当前只生成 `point_to_point` hint，不把多个目标合成 Replicate，也不隐式广播。这样 RuntimePlan V1 不需要加字段：已有 `world_size`、`device_counts`、Value Place 和 Transfer 已能完整表达结果。变化只在导出内容：原 CKKS ValueId 保持不变，通信副本从逻辑 ValueId 之后分配新 ID，target 写入真实拓扑，Encode/外部输入留在 Host，compute 与 Transfer 写入各自 Place。

### 7.5 已定决策:MLIR 原生产出,HEVM 整条路退役
...
1. 编译期,`earth.constant` 直接保留 `DenseElementsAttr` 浮点 slots；frontend 不再运行旧的 ElideConstant Pass，也不再写 `.cst` 索引。Earth→CKKS lowering 把这个 payload 原样交给 `ckks.encode`。`ckks.encode` 声明按输出值的 level/`scale_log2`/NTT 把数据编码成 CKKS 明文。这里不新增 `ckks.constant`,设计见 [Dialect 设计 5.1 节](dialect-design.md);
...
4. Runtime preflight 先核对 bundle id/version/manifest_sha256，再只检查本 rank 实际使用的 blob 的存在性、长度、内容哈希和小端 float64 合法性。manifest 只列原始数据 blob，不保存 ValueId；
...
统一执行器、多 rank 解释框架、显式 Encode 和 `encode_plaintext` 已由 `SequentialRuntime<Api>` 的 Vec/Mock/MPI 路径验证。
...
`PoseidonGpuApi::Value` 要同时容纳四种真实对象：

~~~text
Host plaintext   -> poseidon::Plaintext
Host ciphertext  -> poseidon::Ciphertext
Device plaintext -> poseidon::gpu::GpuPlaintextData
...
不要把 Runtime 的 `ComputeOp` 再翻译成另一套执行 IR。首版直接把 `ComputeKind` 映射到 `GpuEvaluator`：

| Runtime `ComputeKind` | Poseidon GPU 调用 | 首版状态 |
| --- | --- | --- |
| AddCC | `GpuEvaluator::add` | 支持 |
| AddCP | `GpuEvaluator::add_plain` | 支持 |
| SubCC | `GpuEvaluator::sub` | 支持 |
| SubCP | `GpuEvaluator::sub_plain` | 支持 |
| MulCC | `GpuEvaluator::multiply` | 支持 |
| MulCP | `GpuEvaluator::multiply_plain` | 支持 |
...
`PoseidonGpuApi` 先计算 `drop_count = input.level - target_level`，要求它为正数且不超过 OperatorSpec 的 `max_levels_per_op`，然后连续调用 `GpuEvaluator::rescale` 恰好 `drop_count` 次。普通 Rescale 的 `drop_count=1`；当前 lazy Rescale 的 `drop_count=4`，对应约 120 bit。这个循环是一个 RuntimePlan Rescale 的 GPU 实现，不是运行时重新规划，也不是失败后的兜底路径。
...
实现放在 Poseidon 仓库现有目录中：

~~~text
src/poseidon/runtime_api/
|- poseidon_cpu_api.*
|- poseidon_gpu_api.*
`- CMakeLists.txt
~~~

新增 `poseidon_runtime_gpu_api` 静态库和 `poseidon::runtime_gpu_api` alias，继续链接 `poseidon_shared` 与 `ckks_runtime::core`，不增加新的第三方依赖。现有 `POSEIDON_BUILD_CKKS_RUNTIME_API` 选项的说明从“CPU Api integration”改成“CPU/GPU Api integration”。GPU 测试用单独的显式开关控制；用户打开测试但机器没有可用 GPU 时直接失败，不把 skip 当成通过。
...
| Poseidon 后端测试 | GPU 计划 → MockVecApi/PoseidonGpuApi；等价 Host 计划 → PoseidonCpuApi | 三种后端算出同样的数学结果；GPU 值元信息、设备位置和同步语义一致；后续 Host boot 模拟能正确往返 |
...
- Dacapo 当前用确定性 HEFT 完成整值的多 rank、多 device placement，并用 `dist.transfer` 显式化跨 Place 点对点副本；RuntimePlan V1 本身不需要升级;
