...
计划文件只描述"计划"本身,不规定 Poseidon GPU 对象在显存里的内存布局。常量数据(比如模型权重)放在随计划分发的**明文数据包**里,计划顶层用 `plaintext_bundle` 的 id/version/fingerprint 引用;数据包格式(manifest + 内容寻址数据文件)见 [plaintext-bundle.md](../runtime-plan/v1/plaintext-bundle.md)。配套规则是 **IO-2:外部输入一律声明在 Host**——调用方(或数据包加载器)只向 Host 物化数据,权重进设备靠 initialization 阶段的显式 transfer/replicate,执行阶段反复复用这些常驻副本。
...
1. 编译期,IR 里的明文常量用双形态 constant 算子表示(可内联数据、可引用外部数据),设计见 [Dialect 设计 5.1 节](dialect-design.md);
2. 导出前,常量外化 pass 把常量的浮点 slot 向量按内容哈希写入**明文数据包**(去重免费:字节相同的 mask 合并为一个条目)。**外化不做 CKKS 编码**——包里存编码前的浮点数组,编译器不需要链接 Poseidon/SEAL,数据包也比存编码后多项式小一个数量级;
3. 导出 RuntimePlan 时,每个常量变成一条 Host 上的 plaintext external_input,计划顶层写 `plaintext_bundle` 指纹引用,initialization 阶段生成把权重铺到各设备的显式 transfer/replicate;
4. Runtime 加载时做 preflight:清单指纹对得上计划引用(BND-1)、条目集合恰好等于计划的明文外部输入、每个数据文件的哈希和长度对得上清单;
5. 装载时,Api 按计划 ValueDesc 声明的 level/`scale_log2`/`ntt` 把浮点向量编码成 Host 明文(真实后端 CKKS encode;VecApi 直接当 slots 用)——同一份数据包同时服务真实后端和明文 difftest。

低内存联调模式由此免费获得:测试编码器把所有权重映射到少数几个占位常量,数据包缩成几个条目,计划结构(指令数、SSA 图)与真实权重完全一致——difftest 走同一份计划逻辑,只换数据。
