...
**和现状的差距要心里有数**:本仓库的 `ValueDesc`、VecValue 和 MPI 元信息已经包含 context、level、整数 `scale_log2`、NTT 和分量数。当前真正缺的是显式 Encode 指令、inline/bundle payload、bundle 装载入口和完整的 OperatorSpec 校验。V1 协议和计划指纹中的 scale 仍只能用整数 `scale_log2`;允许出现的浮点数只限 Encode 的 inline slot 数据。
...
计划文件只描述"计划"本身,不规定 Poseidon GPU 对象在显存里的内存布局。external_inputs 只表示调用方传入的函数参数。模型权重由 initialization 中的 Encode 指令产生:小 payload 可以直接放在 JSON 中,大 payload 通过 `content` 哈希引用随计划分发的**明文数据包**;数据包格式见 [plaintext-bundle.md](../runtime-plan/v1/plaintext-bundle.md)。数据包的本机目录由部署方启动 Runtime 时另外传入,不写进计划。Encode 输出和 external input 都先位于 Host,进设备必须靠显式 Transfer/Replicate。
...
4. Runtime preflight 先核对 bundle id/version/fingerprint,再检查 manifest `blobs` 中所有文件的存在性、长度、内容哈希和小端 float64 合法性,最后确认每个 Encode 引用的 `content` 及其输出约束。manifest 只列原始数据 blob,不保存 ValueId;
5. Runtime 执行 Encode 时,Api 按输出 ValueDesc 把 inline 或 bundle 浮点向量编码成 Host 明文(真实后端调用 CKKS encode;VecApi 直接当 slots 用),之后计划中的 Transfer/Replicate 再把它铺到设备。
...
Runtime 只面对一个窄接口:编码明文(`encode_plaintext`)、计算(`compute`)、发起异步通信(`communicate_async`)、等待(`wait`)、最终同步(`synchronize`)、出错全组终止(`abort_all`)。`encode_plaintext` 接收 Encode 输出的 ValueDesc 和一段 float64 slots,返回 Host plaintext。Api 内部怎么完成具体编码、计算和通信,Runtime 不关心:
...
Runtime 仓库的协议测试集应包含:inline/bundle 两种 Encode、同一 `content` 不同 level/scale 的两个输出、每种计算和通信指令的最小合法计划;CPU eager 和 GPU lazy 计划;原生 Boot 与 Host `decrypt_reencrypt` Boot;单卡、单机多卡、多进程的代表计划;合法的边界值;以及成对的"必须拒绝"样例——Encode payload 两种形态混写、bundle 缺 content、inline 出现非有限值、Encode 不在 initialization、未知版本、未知 OperatorSpec、spec 指纹不符、`level`/`scale_log2` 不是整数、算子目标元信息和输出 ValueDesc 不一致、重复定义 ValueId、位置对不上、Transfer/Replicate 列表长度不匹配、目标能力不符,等等。指纹的稳定性和浮点 JCS 跨语言一致性也要有专门测试。
...
- `ckks.encode` 在 RuntimePlan 中保留为 initialization 的显式 Encode 指令;external_inputs 只表示调用方函数参数;
