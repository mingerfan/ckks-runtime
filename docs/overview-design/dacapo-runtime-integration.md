...
3. 在 Dacapo 里搭 MLIR 层的 RuntimePlan 产出管线,先支持单 Host 配置,把 `ckks.encode` 一对一导出成 Encode 指令,实现生成端自检和 JSON 写出。已完成：当前 Pass 支持线性单 block CKKS、inline Encode，并覆盖 Dacapo 当前会产生的 CKKS 计算算子;
