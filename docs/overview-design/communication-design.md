...
> 本文描述 Runtime↔Api 的通信契约。V1 已实现 Transfer、Replicate、全部 hint 的合法性检查、`preflight`、Encode、完整值校验、`wait` 和最终 `synchronize`。hint 的具体优化和真实 GPU/NCCL 后端属于后续阶段。
...
Mock/MPI 的 `preflight` 比较 reader 计算的 `plan_source_sha256`，并检查调试开关一致。Runtime 对 external input、Encode、compute 和通信输出都调用完整 `validate_value`。
