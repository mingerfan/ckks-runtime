...
> 本文描述 Runtime↔Api 的目标通信契约。当前代码已实现 Transfer、Replicate、Auto/PointToPoint/Broadcast、`preflight`、`wait` 和最终 `synchronize`;Tree/Ring/HostStaged、显式 Encode 与真实 GPU/NCCL 后端尚未实现。现状见[实现状态](implementation-status.md)。
...
CommAction 来自编译器，runtime 不修改。C++ 字段 `output_types` 使用 `ValueKind`;RuntimePlan JSON 中对应字段名是 `output_kinds`。它只表达明文/密文类别,完整 CKKS 元信息来自对应输出的 ValueDesc。Api 用这些信息在目标端分配或构造值，它们不是底层的 buffer 描述。
...
public:
    ...
    void preflight(
        std::string_view plan_source_sha256,
        bool skip_artifact_digest_checks,
        const TargetConfig &target,
        const OperatorSpec &operator_spec,
        const std::vector<KeyRequirement> &required_keys);

    void validate_value(const Value &value, const ValueDesc &expected);
...
`preflight` 在执行任何计划指令前调用。目标接口传入计划文件完整原始字节的 SHA-256、摘要检查策略和计划要求，Api 对照其他 rank 及自己已配置的 target、capability、context 和密钥逐项检查。`validate_value` 则解决另一个边界问题：Runtime 看不懂 `Api::Value` 的内部结构，所以每次绑定输入、完成计算或收到通信结果后，都要让 Api 按 ValueDesc 核对实际值。

`validate_value` 只能读取句柄创建时已经确定的类型和 CKKS 元信息，不能读取 slots、等待 future/event 或触发设备同步。异步计算返回的 Value 句柄因此必须立刻携带这些不可变元信息，数值结果可以稍后完成。

当前 Mock/MPI 的 `preflight` 仍只比较 `RuntimePlan::fingerprint()` 生成的内部 64 位摘要，尚未改为读取阶段计算的 `plan_source_sha256`；当前 Runtime 也只检查返回值的明文/密文类别。现状见[文档入口](../README.md#代码对应关系)。

`skip_artifact_digest_checks=true` 时 Api 不比较各 rank 的计划摘要,但仍检查这个开关在执行组内一致,并继续完成 target/context/密钥检查。该开关只用于调试,不能由计划文件设置。
...
**启动前**：部署方配置 Api 的 context 和密钥;Runtime 核对目标环境、计划摘要和这些配置。

**初始化**：绑定调用方的 Host external_inputs；执行 Encode 生成 Host 明文常量；把本次运行需要的值预加载到设备；等待初始化通信完成。

**执行**：执行计算；执行中间的 Transfer/Replicate；消费 Pending 输出前调用 `wait`。

**收尾**：下载要求的结果；等待所有未完成的发送句柄；同步并复制最终结果/测试产物；释放 Runtime 内部对象；最后返回已经独立保存的结果。
...
当前完整 Api 与通信相关的接口保留五个函数：

~~~text
preflight(plan_source_sha256, skip_artifact_digest_checks,
          target, operator_spec, required_keys)
...
每个模拟 rank 使用独立的 `MockVecApi` 和 runtime。各 Api 实例只共享一个线程安全的 `MockCluster`（消息队列 + 终止状态），不共享值存储或 `Api::Value`。`MockVecApi` 内部组合 `VecExecutor` 完成计算。

`MockVecApi` 实现同一套完整接口：
