...
public:
    ...
    void preflight(
        std::string_view protocol_fingerprint,
...
`preflight` 在执行任何计划指令前调用。目标接口传入协议 SHA-256 和计划要求，Api 对照自己已配置的 target、capability、context 和密钥逐项检查。`validate_value` 则解决另一个边界问题：Runtime 看不懂 `Api::Value` 的内部结构，所以每次绑定输入、完成计算或收到通信结果后，都要让 Api 按 ValueDesc 核对实际值。

`validate_value` 只能读取句柄创建时已经确定的类型和 CKKS 元信息，不能读取 slots、等待 future/event 或触发设备同步。异步计算返回的 Value 句柄因此必须立刻携带这些不可变元信息，数值结果可以稍后完成。

当前 Mock/MPI 的 `preflight` 仍只比较 `RuntimePlan::fingerprint()` 生成的内部 64 位摘要，尚未迁移到协议 SHA-256；当前 Runtime 也只检查返回值的明文/密文类别。两种指纹的区别见[文档入口](../README.md#代码对应关系)。
...
~~~text
preflight(protocol_fingerprint, target, operator_spec, required_keys)
