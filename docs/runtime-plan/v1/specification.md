...
16. **[BND-1] 出现 bundle Encode 时,每个 Runtime 实例都必须取得顶层引用的完整数据包;`bundle_format_version = 1`,id/version/fingerprint 必须相符,每个 Encode 引用的 content 必须在 manifest 中。manifest `blobs` 列出的每个数据文件都必须存在,长度、内容哈希和小端 float64 数据合法性全部通过检查;再按引用它的每个 Encode 输出分别检查 slot 容量。不存在 bundle Encode 时不允许出现 `plaintext_bundle`**;
