...
> **状态:设计草案。** 本文已经采用显式 Encode 指令和 inline/bundle 双 payload;当前 `schema.json`、样例集和 C++ 读取/执行代码仍是上一版实现,尚未同步。完成 Schema、reader、verifier、Runtime、Api 和端到端测试之后,V1 才会正式冻结。
...
- `external_inputs`:调用方在运行开始时提供的函数参数 ValueId 列表,可以是 plaintext 或 ciphertext。它与明文数据包没有对应关系。**每个外部输入的 place 必须是 Host**——调用方只向 Host 绑定已经物化好的 CKKS 值,数据进设备的唯一途径是计划中显式的通信指令(通常放在 `initialization` 阶段);
...
- `id` / `version`:数据包的标识和版本,同一 `id` + `version` 的内容永远不变;
- `fingerprint`:数据包 `manifest.json` 的指纹,按第 8 节同样规则计算。
...
计划只保存数据包的身份,**不保存本机文件路径**。启动每个 Runtime 实例时,部署方必须另外传入一个该 rank 可读的完整 bundle 目录;各 rank 的目录路径可以不同,但 manifest 的 id、version 和 fingerprint 必须与计划一致。计划含 bundle Encode 却没有传目录,或目录不完整,都在执行任何指令前直接报错。V1 不支持“缺文件时再向别处下载”之类的兜底行为。
...
12. **[SPEC-1] OperatorSpec 的 id/version/fingerprint 与 Runtime 持有的副本相符**,计划中的 level 范围、boot profile 引用、rescale 降幅都在 spec 声明的合法范围内;
...
16. **[BND-1] 出现 bundle Encode 时,每个 Runtime 实例都必须取得顶层引用的完整数据包;id/version/fingerprint 必须相符,每个 Encode 引用的 content 必须在 manifest 中。manifest `blobs` 列出的每个数据文件都必须存在,长度、内容哈希和小端 float64 数据合法性全部通过检查;再按引用它的每个 Encode 输出分别检查 slot 容量。不存在 bundle Encode 时不允许出现 `plaintext_bundle`**;
17. **[FP-1] 重算指纹与 `fingerprint` 字段一致**。
...
3. 按 **RFC 8785(JSON Canonicalization Scheme)** 完整序列化。因为 inline Encode 允许 float64,实现必须遵守 RFC 8785 的数字规范化规则,普通的 `json.dumps`、`nlohmann::json::dump` 或“保持输入文本”都不能直接当作等价实现。不同语言的生成器和读取器必须用共同测试向量验证 1、1.0、指数形式和舍入边界得到相同 canonical bytes;
4. 对 canonical UTF-8 字节算 SHA-256,字段值写成 `"sha256:" + 64 位小写十六进制`。
