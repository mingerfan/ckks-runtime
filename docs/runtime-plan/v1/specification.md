...
- `plaintext_bundle`:**可选**,预编码明文数据包(权重包)的引用,见 3.1 节。缺席表示计划不依赖任何预编码数据包;
...
神经网络推理这类计划的明文输入(权重、bias、rotate-and-sum 的 mask)在编译时就已编码完成,打包成随计划分发的**明文数据包**(bundle)。计划文件本身**不含任何明文数据**——和密钥一样,这里只放引用:
