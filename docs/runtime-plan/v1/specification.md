...
- `external_inputs`:由调用方在启动时提供的 ValueId 列表。**每个外部输入的 place 必须是 Host**——调用方只向 Host 物化数据,数据进设备的唯一途径是计划中显式的通信指令(通常放在 `initialization` 阶段);
...
### 3.1 plaintext_bundle:预编码明文包引用(可选)
...
神经网络推理这类计划的明文输入(权重、bias、rotate-and-sum 的 mask)数值在编译时就已确定,以**编码前的浮点 slot 向量**形式打包成随计划分发的**明文数据包**(bundle);浮点 → CKKS 明文的编码由 Runtime 在装载时按该值的 ValueDesc 执行。计划文件本身**不含任何明文数据**——和密钥一样,这里只放引用:

- `id` / `version`:数据包的标识和版本,同一 `id` + `version` 的内容永远不变;
- `fingerprint`:数据包 `manifest.json` 的指纹,按第 8 节同样规则计算。

数据包的文件格式(manifest、数据文件、内容寻址去重)见 [plaintext-bundle.md](plaintext-bundle.md)。字段缺席表示计划的明文外部输入(如果有)由调用方自行准备,Runtime 不做数据包核对;字段存在且 Runtime 持有同 id 的数据包副本时,指纹不符即拒绝(BND-1)。
...
14. **[IO-2] 每个 external_input 的 place 必须是 Host**:调用方只向 Host 物化数据,进设备必须走显式通信指令;
15. **[BND-1] `plaintext_bundle` 存在且 Runtime 持有同 id 数据包副本时,version/fingerprint 必须相符**(与 SPEC-1 同模式,属于执行前 preflight);
16. **[FP-1] 重算指纹与 `fingerprint` 字段一致**。
