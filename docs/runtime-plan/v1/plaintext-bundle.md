...
- **复用**:同一个数据包既可以喂真实后端(执行 Encode 时做 CKKS 编码),也可以原样喂 VecApi 明文 difftest,两条路径吃同一份字节。
...
基础编码规则与 RuntimePlan 相同(UTF-8 无 BOM、未知字段拒绝、64 位标识符用十进制字符串),但 manifest 本身没有 Encode inline payload,因此**不允许任何浮点数字面量**。浮点数据全部在 `data/*.bin` 里以原始字节存在,不经过 manifest JSON。
...
- `blobs`:数据包包含的原始浮点数据清单,同一 `content` 在清单中必须恰好出现一次:
  ...
  - `byte_length`:数据文件应有的字节数,用于在解码前发现截断、错文件或多余数据。它必须大于 0 且是 8 的倍数;slot 数就是 `byte_length / 8`。
...
装载语义:Runtime 在 preflight 中按每条 bundle Encode 的 `payload.content` 找到清单条目和数据文件,校验并解码成 float64 向量。真正执行 Encode 时,Runtime 再把这段向量交给 Api,由 Api 按输出 ValueDesc 声明的 `level`、`scale_log2`、`ntt` 和 context 参数编码成 Host 上的明文(真实后端调用 CKKS encode;VecApi 直接把数组当 slots 用,不做密码学编码)。
...
1. 清单的 `bundle_id`、`version` 和 fingerprint 分别与计划 `plaintext_bundle.id`、`version`、`fingerprint` 相符(BND-1);
...
external_inputs 与 manifest 没有对应关系:external_inputs 是调用方传入的函数参数,manifest 是 Encode 使用的模型数据清单。值的 CKKS 元信息(level、`scale_log2`、`ntt`、`components`)只在 Encode 输出的 ValueDesc 里声明——清单管原始字节,计划管编码语义。
