...
Api 负责：定义值类型、实现各计算函数、实现 `communicate_async` 和 `wait`、区别对待 Host 和 Device、选择或降级到等价的传输方式、抛出带细节的错误、实现 `abort_all`。
...
> 计算函数返回后，它的输出对**同一个 Api 实例上后续发起的调用**保证可见（包括交给 `communicate_async`）。真正阻塞到 host 的同步点只有 `wait`。
