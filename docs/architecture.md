...
- 一个只负责执行的 runtime；
- 用普通向量模拟密文运算的 VecApi；
- 用线程和消息队列模拟通信的 MockCommunicationApi；
...
8. Api 兼容层负责具体的数据对象、通信协议，以及等价的实现降级。
...
- 用 Transfer / Replicate / Gather 等通信动作表达搬运
- 带物理的来源 / 目标 Place
- 可选的通信实现提示（hint）
        |
        | 验证 + 翻译
...
Runtime 不持有也不搜索这份拓扑模型。可执行计划里已经写死了物理 Place，runtime 只需要知道"我是哪个 rank"，并在启动时检查实际的节点数和本地设备数是否和编译目标一致。
...
原始的逻辑 CKKS 可以保留。可执行计划可以继续用 MLIR，也可以翻译成 C++ 的执行序列。
...
Runtime 不理解：值的实际 C++ 类型、一个密文有几个分量、数据在内存里是否连续、目标怎么分配、用的是 MPI 还是 NCCL 还是 CUDA 点对点、通信怎么降级、CPU/GPU 表示怎么转换。

Api 负责：定义值类型、实现各计算函数、实现 `communicate_async`、`wait` 和最终输出 `synchronize`、区别对待 Host 和 Device、选择或降级到等价的传输方式、抛出带细节的错误、实现 `abort_all`。
...
外部输入、明文常量、密文输入都可以一开始放在 Host。要不要搬到设备上，由 Place 决定，而不是由它是明文还是密文决定：
...
从 Host 搬到 Device 的这一步里，Api 内部可以顺便完成：锁页/非锁页的暂存、CPU/GPU 格式转换、显存分配、cudaMemcpy、元信息转换。这些细节都不进入 runtime。
...
初始化阶段把本次执行需要的外部值和明文常量全部预加载到目标设备，并一直保留到运行结束。这样实现简单，代价是内存占用偏高。
...
MPI 版 Api 可以用 `MPI_Abort`；单进程的 VecApi 可以抛出 ClusterPanic 或直接终止。
...
1. 所有计算指令都是纯计算，无副作用。
2. 每条计算指令只有一个执行 Place。
3. 计算指令的输入已经在执行 Place 上就绪。
4. 不存在隐式的跨 Place 操作数。
5. 所有数据搬运都由显式的通信动作表示。
6. 通信动作的数学语义和结果类型都明确。
7. 来源/目标 Place 都来自编译目标。
8. 传输 ID 在同一个计划里稳定且唯一。
9. 所有 rank 使用同一份可执行计划。
10. 使用一个值之前，必须先把它从 Pending 等成 Ready。
11. Api 的实现降级不得改变通信动作的数学语义。
12. 任意错误都会终止整个执行组。
...
> 计算函数返回后，它的输出对**同一个 Api 实例上后续发起的调用**保证可见（包括交给 `communicate_async`）。指令执行期间阻塞 host 的同步点只有 `wait`；运行结束发布最终输出前还会调用一次 `synchronize`。

这句话的用意是：Api 内部可以用 CUDA stream/event 给算子和通信排序，只要保证"后一个调用能看到前一个的结果"即可，不必在每个算子后面都做一次 `cudaStreamSynchronize`。Runtime 的代码完全不受影响——它照样先算、要用的时候再 `wait`。对 VecApi 这种同步实现，这个契约天然满足。
...
- **同步实现的 Api 没有重叠**。VecApi 的默认同步模式在 host 线程上当场算完，多卡模拟在它之下是纯串行的——这不影响其验证正确性的用途。为了用明文模拟真实的并行时序，VecApi 另提供异步模式：每个模拟设备一个工作线程，`compute` 入队即返回，接口与同步模式完全相同（设计见测试文档 2.1 节）。
...
- 固定目标拓扑；
- Host 和 Device Place；
- VecApi（同步与异步两种模式）和 MockCommunicationApi；
